({
    ...
    setIframe : function(cmp) {
        ...
        var element = cmp.find('iframe').getElement();
    ...
    handleUrl: function(cmp, openActionIn, url, action, actionConfig) {
        ...
        if (/^\/[a-zA-Z0-9]{15,18}(\?.*)*$/.test(url)) {
            ...
            if (workspaceAPI && this.isConsoleNavigation) {
                return workspaceAPI.getEnclosingTabId().then(function(response) {
    ...
    handleOmniNavigation: function(cmp, type, destination, additionalConfig) {
        /* We need to get tabId if we're going to close it */
        var helper = this;
        var workspaceAPI = helper.workspaceAPI;
        if (workspaceAPI && helper.isConsoleNavigation && additionalConfig.closeTab) {
            workspaceAPI.getEnclosingTabId()
                .then(function(response) {
                    var promise = helper.doOmniNav(cmp, type, destination, additionalConfig);
            ...
            helper.doOmniNav(cmp, type, destination, additionalConfig);
        }
    },
    ...
    doOmniNav: function(cmp, type, destination, additionalConfig) {
        ...
        switch (type) {
            ...
            default:        return this.handleUrl(cmp, additionalConfig.openActionIn, destination, defaultActionConfig, additionalConfig.actionConfig);
        }
    },
